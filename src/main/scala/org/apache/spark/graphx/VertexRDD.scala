...
abstract class VertexRDD[VD](
   ...
   * 对当前RDD的每个分区进行函数变换得到一个新的VertexRDD
   */
   ...
   *根据VertexId做差集， VertexRDD[VD]和RDD[(VertexId, VD)]底层存储是一致的，
   * VertexRDD[VD]是RDD[(VertexId, VD)]的子类
   ...
   *去掉this和other中有相同值的点。如果冲突留下other的
  ...
  /**
   * Left joins this RDD with anotther VertexRDD with the same index. This function will fail if
   * both VertexRDDs do not share the same index. The resuling vertex set contains an entry for
   * each vertex in `this`.
   * 对相同index的元素做连接，左边有而右边没有的，右边返回None
   * If `other` is missing any vertex in this VertexRDD, `f` is passed `None`.
   *
   * @tparam VD2 the attribute type of the other VertexRDD
   *             other VertexRDD中属性的类型
   ...
   * the vertex is picked arbitrarily.如果有重复会从中随意挑选一个。
   *
   * @tparam VD2 the attribute type of the other VertexRDD
   * @tparam VD3 the attribute type of the resulting VertexRDD
   *
   ...
   * @param f the function mapping a vertex id and its attributes in this and the other vertex set
   * to a new vertex attribute.
   * @return a VertexRDD containing all the vertices in this VertexRDD with the attributes emitted （发出，放出）
   * by `f`.
   */
