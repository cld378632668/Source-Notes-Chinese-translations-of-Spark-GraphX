...
class EdgePartition[
  ...
  /** Return a new `EdgePartition` with the specified edge data. 用给定的边的数据返回一个新的'EdgePartition'.*/
  ...
  /** Return a new `EdgePartition` with the specified active set, provided as an iterator.
    * 用给定的活跃集(作为一个参数提供)返回一个新的'EdgePartition'.*/
  ...
  /** Return a new `EdgePartition` without any locally cached vertex attributes.
    * 返回一个没有本地缓存的点属性的新的'EdgePartition.'*/
  ...
  /**
   * Reverse all the edges in this partition.所有的边反向.
   *
   ...
   * 对所有的边应用用户定义的函数构造一个新的EdgePartition
   ...
   * vertices match `vpred`.
    * 通过对原有的EdgePartition进行过滤,构造一个新的边分区,边要使epred函数返回值为true,顶点要使vpred返回值为true.
   */
  def filter(
   ...
   * 通过用户给定的merge函数,把所有具有相同起始和终止顶点的边合并成一条边.
   ...
   * 对当前EdgePartition和传入的EdgePartion中的所有边应用函数f.
    * 对两个分区中的边做内连接操作,概念类似于关系型数据库中的内连接操作.
   ...
   *如果当前EdgePartition有许多具有相同起始和终止顶点的边,'f'将对所有的这些边都应用一次, 但是每次调用时会使用任意一条'other'中相应的边.
   * If there are multiple edges with the same src and dst in `other`, `f` will only be invoked
   * once.
    * 如果'other'中有多条具有相同顶点的边,f只会被调用一次.
   */
   ...
   *沿着边传递信息,并在接收的顶点们那里聚集他们.通过有次序的扫描所有边来实现.
   ...
   * 沿着边传递信息,并在接收的顶点们那里聚集他们.实现方式:过滤其实顶点的id,然后扫描每一个边聚簇.
   * @param sendMsg generates messages to neighboring vertices of an edge
    *                函数,给一条边的邻居顶点产生信息
   * @param mergeMsg the combiner applied to messages destined to the same vertex
    *                 函数,合并目的地是相同顶点的信息
